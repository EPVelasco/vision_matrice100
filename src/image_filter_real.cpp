...
void callback(const ImageConstPtr& in_image)
{
  ...
  if (contleft >0 and contrigth >0 ){
    ...
    float angle_lin1 =  std::atan2(lin_group1[3] - lin_group1[1], lin_group1[2] - lin_group1[0]);
    float angle_lin2 =  std::atan2(lin_group2[3] - lin_group2[1], lin_group2[2] - lin_group2[0]);

    xc = (lin_group1[0] + lin_group1[2]+lin_group2[0] + lin_group2[2])/4;
    yc = imageHeight/2;

   // xc = (max_x+min_x)/2;
    float angle_average = (angle_lin1+angle_lin2)/2;

    std::cout<<"Angle: " << angle_average<<std::endl;
    ...
    xc = 0;
    yc = 0;
    std::cout<<"[ERROR]: no hay suficientes lineas"<<std::endl;
  }
  ...
  cv::circle(resultImage, cv::Point(int(xc),int(yc)), 3, cv::Scalar(255, 255, 0),-1); 

  cv::line(resultImage, cv::Point(lin_group1[0], lin_group1[1]), cv::Point(lin_group1[2], lin_group1[3]), cv::Scalar(0, 0, 255), 5, cv::LINE_AA);
  cv::line(resultImage, cv::Point(lin_group2[0], lin_group2[1]), cv::Point(lin_group2[2], lin_group2[3]), cv::Scalar(0, 255, 0), 5, cv::LINE_AA);
